        ...
        async function joinNetworkRoom(roomId) {
            ...
            roomUnsubscribe = onSnapshot(roomRef, (snapshot) => {
                ...
                if (data && data.players) {
                    ...
                    Object.keys(data.players).forEach(pId => {
                        if (pId !== myPlayerId) {
                            ...
                            if (pData && (now - (pData.updatedAt || 0) < 15000)) {
                                if (!remotePlayers[pId]) {
                                    audio.playJoin();
                                    showToast(`Игрок ${pData.name} вошел в комнату!`);
                                    addChatMessage('Система', `${pData.name} присоединился`, 'system');
                                }
                                remotePlayers[pId] = pData;
                            } else {
                ...
                if (data && Array.isArray(data.chatMessages)) {
                    ...
                    if (newMessages.length > chatLogs.length) {
            ...
            setInterval(async () => {
                if (currentRoomId && db && isAuthReady) {
                    try {
                        const playerUpdate = {};
                        playerUpdate[`players.${myPlayerId}`] = {
                ...
                if (msg.type === 'system') {
                    ...
                    div.innerHTML = `
                        <div class="flex items-center gap-1">
                            ...
                            <span class="text-neutral-200 break-words">${msg.text}</span>
                        </div>
                    `;
                }
